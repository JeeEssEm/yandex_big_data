...
def convert_to_datetime(str_datetime):
  return datetime(int(str_datetime[:4]), int(str_datetime[5:7]), int(str_datetime[8:10]), int(str_datetime[11:13]))
...
class Prophet_anomaly:
    def __init__(self, dataset_name):
        self.dataset_name = dataset_name
        self.pyspark_data()
        self.df = pd.read_csv(dataset_name, sep=',')
    def result(self):
      global fig_list
